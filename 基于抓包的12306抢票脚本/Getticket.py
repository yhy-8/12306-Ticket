...
class GetTicket:
    ...
    def get_ticket_info(self):
        ...
        try:
            ...
            print('获取车票信息成功')
            mylog.logrecord("get_tickets_url", url, self.logswitch)
            mylog.logrecord("get_tickets_response",self.tickets,self.logswitch)
            return 1
    ...
    def init_order(self):
        ...
        mylog.logrecord("init_order_url",url,self.logswitch)
        mylog.logrecord("init_order_request_data",data,self.logswitch)
        #mylog.logrecord("init_order_response_text",response.text,self.logswitch) #这里的response会返回一个网页
    ...
    def check_passengers(self):
        ...
        response = self.session.post(url, data=data)
        mylog.logrecord("check_passengers_url", url, self.logswitch)
        mylog.logrecord("check_passengers_request_data", data, self.logswitch)
        mylog.logrecord("check_passengers_response_text", response.text, self.logswitch)
        if response.status_code == 200:
            try:
    ...
    def check_order_info(self):
        ...
        while times < 10:
            ...
            response = self.session.post(url, data=data)
            mylog.logrecord("check_order_info_url", url, self.logswitch)
            mylog.logrecord("check_order_info_request_data", data, self.logswitch)
            mylog.logrecord("check_order_info_response_text", response.text, self.logswitch)
            if response.status_code == 200:
                if self.sysbusy in response.text:
                    print("服务器繁忙！正在重试！")
                    continue
    ...
    def submit_order(self,train):
        ...
        while times < 10:
            times = times + 1  # 最多重复10次
            response = self.session.post(url, data=data)
            mylog.logrecord("submit_order_url", url, self.logswitch)
            mylog.logrecord("submit_order_request_data", data, self.logswitch)
            mylog.logrecord("submit_order_response_text", response.text, self.logswitch)
            if self.sysbusy in response.text:
                print("服务器繁忙！正在重试！")
                continue
            else:
    ...
    def confirm_order(self, train):
        ...
        while times < 10:
            times = times + 1  # 最多重复10次
            response = self.session.post(url, data=data)
            mylog.logrecord("confirm_order_url", url, self.logswitch)
            mylog.logrecord("confirm_order_request_data", data, self.logswitch)
            mylog.logrecord("confirm_order_response_text", response.text, self.logswitch)
            if response.status_code != 200:
                print(f"请求失败！状态码：{response.status_code}  正在重试！")
                continue
    ...
    def base_log(self):
        ...
        while True:
            response = self.session.post(url, data=data)
            mylog.logrecord("base_log_url", url, self.logswitch)
            mylog.logrecord("base_log_request_data", data, self.logswitch)
            mylog.logrecord("base_log_response_text", response.text, self.logswitch)
            if response.status_code == 200:
                print("记录日志成功！")
                break
            else:
    ...
    def queue_order(self):
        ...
        while times<10:
            ...
            mylog.logrecord("queue_order_url", url, self.logswitch)
            mylog.logrecord("queue_order_response_text", response.text, self.logswitch)
            if response.status_code == 200:
                data = response.json()
                # 获取排队等待时间
